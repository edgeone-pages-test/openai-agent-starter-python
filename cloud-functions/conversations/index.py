...
class handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            ...
            logger.log(
                f"list_conversations: store returned {len(raw_items)} raw item(s); "
                f"first item keys={list(raw_items[0].__dict__.keys()) if raw_items and hasattr(raw_items[0], '__dict__') else (list(raw_items[0].keys()) if raw_items and isinstance(raw_items[0], dict) else None)!r}"
            )
            conversations = [
